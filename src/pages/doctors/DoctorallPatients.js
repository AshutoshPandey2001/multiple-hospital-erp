        ...
        <div style={{ width: '800px', marginRight: '50px' }} >
            <div className="card" style={{ border: "2px solid black" }}>
                ...
                <div className='row' style={{ padding: '20px' }}>
                    ...
                    <div className='col-lg-6 col-md-6 col-12 '>
                        <span><b>Follow up: {state.followup}</b></span>
                    </div>
...
const DoctorallPatients = () => {
    ...
    const subcollectionRef = parentDocRef.collection('opdPatient').where('hospitaluid', '==', hospitaluid).where('deleted', '==', 0).where('druid', '==', druid).where('paymentStatus', "==", "Completed");
    ...
    const columns = [
        ...
        { name: 'Mobile No', selector: row => row.pMobileNo ? row.pMobileNo : '-' },
        {
            name: 'Action', cell: row => <span style={{ display: 'flex', justifyContent: 'center' }}>
                <button style={{ color: 'skyblue', border: 'none' }} onClick={() => navigate('/allcheckpatients/patientshistory', { state: row })} >{row.description ? <AiFillPrinter size={25} /> : <BsEye size={25} />} </button >
